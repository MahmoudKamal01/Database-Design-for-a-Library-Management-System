...
def viewFaCU(showFrame, root):
    ...
    def CurSelet(evt):
        if L1.curselection() != ():
            ...
            author = authors[0]  
            books = database.ReadLine("select * from books where faculty_id =?",[author])
            L2.delete(0,END)
            L2.insert(books[0], books[1])
...
def viewCate(showFrame, root):
    ...
    def CurSelet(evt):
        if L1.curselection() != ():
            ...
            author = authors[0]  
            books = database.ReadLine("select * from books where category_id =?",[author])
            L2.delete(0,END)
            L2.insert(books[0], books[1])
            L2.place(x=400, y=i+80)
            L2.bind('<<ListboxSelect>>', CurSelet1)
...
def viewAuth(showFrame, root):
    ...
    def CurSelet(evt):
        if L1.curselection() != ():
            ...
            author = authors[0]  
            books = database.ReadLine("select * from books where author_id =?",[author])
            L2.delete(0,END)
            L2.insert(books[0], books[1])
            L2.place(x=400, y=i+80)
            L2.bind('<<ListboxSelect>>', CurSelet1)
...
#def addEve(event_name,author_name,ticket_price,event_date,success):
